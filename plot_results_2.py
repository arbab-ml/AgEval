...
#                 embedding_values = [result_table_dict[shot].loc[(model, dataset), f'Embedding Same Category Count {shot}'] for shot in shots]
...
#                 if encoder == 'clip':
#                     random_values = [result_table_dict[shot].loc[(model, dataset), f'Random Same Category Count {shot}'] for shot in shots]
...
#         ax.set_ylabel('Same Category Count')
...
#     plt.savefig(os.path.join(plots_folder, 'same_category_count_plots.pdf'), bbox_inches='tight')
#     plt.close()

#     print(f"Plots saved in '{plots_folder}/same_category_count_plots.pdf'")
